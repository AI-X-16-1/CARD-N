...
async def get_graph(
    ...
    return await service.get_graph(depth=depth, job_filter=job_filter)


@router.post("/{person_id}/introduction-requests", status_code=status.HTTP_201_CREATED)
async def request_introduction(
    person_id: int, service: GraphService = Depends(_service)
) -> IntroductionRequestResponse:
    return await service.request_introduction(person_id)
...
async def get_introduction_request(
    ...
    return await service.get_introduction_request(person_id)
...
async def list_introduction_requests(
    ...
    return await service.list_incoming_requests()


@router.post("/introduction-requests/{person_id}/approve")
async def approve_introduction_request(
    person_id: int, service: GraphService = Depends(_service)
) -> IntroductionRequestResponse:
    return await service.respond_to_request(person_id, approve=True)


@router.post("/introduction-requests/{person_id}/decline")
async def decline_introduction_request(
    person_id: int, service: GraphService = Depends(_service)
) -> IntroductionRequestResponse:
    return await service.respond_to_request(person_id, approve=False)
...
async def add_acquaintance(
    person_id: int,
    body: AddAcquaintanceRequest,
    service: GraphService = Depends(_service),
) -> AcquaintanceResponse:
    return await service.add_acquaintance(person_id, name=body.name, job_class=body.job_class)


@router.get("/{person_id}/acquaintances")
async def list_acquaintances(
    person_id: int, service: GraphService = Depends(_service)
) -> AcquaintancesResponse:
    return await service.list_acquaintances(person_id)


@router.post("/acquaintances/{acquaintance_id}/consent")
async def record_acquaintance_consent(
    acquaintance_id: int, service: GraphService = Depends(_service)
) -> AcquaintanceResponse:
    return await service.record_acquaintance_consent(acquaintance_id)


@router.get("/stats")
async def get_graph_stats(service: GraphService = Depends(_service)) -> GraphStatsResponse:
