...
// Every endpoint requires X-Device-Id (backend/app/device.py) — without it the server
// cannot tell whose contacts these are, and answers 422 rather than guessing. Attached
// here rather than at each call site so a new request cannot forget it: the id is read
// from storage once and memoized, so this costs nothing after the first request.
